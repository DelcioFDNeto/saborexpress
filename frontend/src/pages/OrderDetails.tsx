...
export default function OrderDetails() {
  ...
  const fetchOrderDetails = async () => {
    ...
    try {
      ...
      const tableRes = await axios.get(`${apiUrl}/api/tables/${tableId}`);
      ...
      const orderRes = await axios.get(`${apiUrl}/api/orders/${activeOrderData.id}`);
  ...
  useEffect(() => {
    fetchOrderDetails();
    // Poll for updates every 15 seconds to check if items are ready
  ...
  const handleCloseRequest = async () => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      await axios.post(`${apiUrl}/api/tables/${tableId}/close-request`);
      alert('Fechamento solicitado! A mesa está travada aguardando o caixa.');
      fetchOrderDetails();
    } catch (err: any) {
      console.error(err);
      alert(err.response?.data?.message || 'Erro ao pedir fechamento.');
    }
  };
  ...
  const handleTableActionSubmit = async (targetTableId: number) => {
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
      const endpoint = tableActionType === 'transfer' ? 'transfer' : 'merge';
      
      const res = await axios.post(`${apiUrl}/api/tables/${tableId}/${endpoint}`, {
      ...
      console.error(err);
      alert(err.response?.data?.message || 'Erro ao realizar a operação.');
    }
  };
      ...
      <div className="w-full md:w-96 bg-white border-l border-gray-100 p-8 flex flex-col shadow-[-10px_0_20px_-10px_rgba(0,0,0,0.05)] z-10 relative">
        ...
        <div className="space-y-4 flex-1">
          ...
          <div className="flex justify-between text-gray-600">
            ...
            <span className="font-medium text-emerald-600">+ R$ {(Number(order.total_amount) * 0.1).toFixed(2)}</span>
          </div>
          ...
          {canAddItems && (
            <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-100">
              ...
              <button 
                ...
                className="py-3 rounded-xl border-2 border-emerald-100 text-emerald-600 font-bold hover:bg-emerald-50 transition-colors text-sm flex items-center justify-center gap-1"
              >
                Juntar
              </button>
            </div>
